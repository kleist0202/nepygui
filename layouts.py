...
class GridLayout(Layout):
    def __init__(self, screen, orientation="NW", x_start=0, y_start=0):
        ...
        self.orientation = orientation
        self.x_size, self.y_size = screen.get_size()
        self.saved_padding_x = x_start
        self.saved_padding_y = y_start
        self.layouts = {}
        #hl = HLayout(screen, self.orientation, x_start, y_start)

    def add_widget(self, widget, row, col, xpadd=0, ypadd=0):
        if (widgets := self.layouts.get(row)) is None:
            widgets = HLayout(self.screen, orientation=self.orientation,
                              x_start=self.saved_padding_x, y_start=self.saved_padding_y)
            self.layouts[row] = widgets
            self.saved_padding_y += widget.get_size()[1] + ypadd
        widgets.add_widget(widget, xpadd)

    def draw(self, screen_size, mouse_pos, mouse_button, keys, delta_time):
        for num, layout in self.layouts.items():
            layout.draw(screen_size, mouse_pos, mouse_button, keys, delta_time)
